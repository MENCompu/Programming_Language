...
typedef enum {
    ...
    TIT_Run_Length_Compressed_True_Color      = 11,
    TIT_Run_Length_Compressed_Black_And_White = 12,
} TGA_Image_Type;
...
Public b8 Image_Converter_TGA(Memory_Arena* arena, Image* image, Slice_byte mem) {
    TGA_Header* header;
    header = (TGA_Header*)mem.E;

    *image = (Image){
        .dims = { header->imageSpec.dims.width, header->imageSpec.dims.height },
        .channelCount = 4,
        .channelSizes = { 8, 8, 8, 8 },
        .data = (byte*)mem.E + (sizeof(TGA_Header) + header->lengthID + header->colorMap.length),
    };
    ...
    switch ( (TGA_Image_Type)header->imageType ) {
        ...
        case TIT_Uncompressed_True_Color: {
            u32* reformatedData = Arena_Alloc_Array(arena, u32, Image_Pixel_Count(image));

            if ( bitDepth == 24 ) {
                Image_BGR_To_RGBA(image->data, Image_Pixel_Count(image), reformatedData);
                image->hasTransparency = false;
            } else { // bitDepth == 32
                Image_BGRA_To_RGBA(image->data, Image_Pixel_Count(image), reformatedData);
                image->hasTransparency = Image_Has_Transparency(reformatedData, (u32)(image->dims.width * image->dims.height));
            }

            image->data = reformatedData;
        } break;
        ...
        case TIT_Uncompressed_Black_And_White: {
            ...
            u32* reformatedData = Arena_Alloc_Array(arena, u32, Image_Pixel_Count(image));
            Image_Grey_Scale_To_RGBA(image->data, Image_Pixel_Count(image), reformatedData);
            image->hasTransparency = true; // TODO(JENH): Grey scale should always be transparent?.
            image->data = reformatedData;
        } break;
    ...
    if ( Flags_Has_All(header->imageSpec.imageDescriptor, TOP_TO_BOTTOM_ORDERING_FLAG) ) {
        Image_RGBA_Flip_Vertically(image->data, image->dims.width, image->dims.height);
    }
...
#if 0
Public void Image_Load(Memory_Arena* arena, Image* image, ) {
    Image_Converter_TGA(Memory_Arena* arena, Image* image, Slice_byte mem);
}
#endif
